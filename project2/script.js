...
function startGame() {
    ...
    let Images = imgElementsArray;


    for(i=0; i<Images.length; i++) {
        cardElements[i].innerHTML = "";
        cardElements[i].appendChild(Images[i]);
        cardElements[i].type = `${Images[i].alt}`;
    ...
    moves = move+1;
    counter.innerText = `Remaining moves:${moves} `;
...
function moveCounter() {
    moves--;
    counter.innerHTML = `Remaining Moves : ${moves} `;
